/**
 * GET  /api/messages/[id] — read one facility conversation
 * POST /api/messages/[id] — reply to it
 */
import { NextRequest, NextResponse } from "next/server";
import { z } from "zod";
import { auth } from "@/lib/auth";
import { getThread, replyToThread } from "@/lib/messages/service";
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  const result = await replyToThread(session.user.id, id, parsed.data.body);
  if ("error" in result) {
